...
const fetchLeaseSummary = async (apartmentNumber: string): Promise<LeaseSummary> => {
  try {
    const response = await fetch(`/api/lease/summary?apartmentNumber=${apartmentNumber}`);
          ...
          try {
            ...
            if (summary) {
              const simpleLease: LeaseDocument = {
                ...
                leaseStatus: summary.leaseStatus as "ACTIVE" | "EXPIRED" | "PENDING"
              };
            ...
            <div className="w-full max-w-md bg-white rounded-lg shadow-md flex flex-col items-center justify-center p-6 border border-gray-300 mb-6">
              ...
              <div className="w-full space-y-4 mt-4">
                <div className="border-b border-gray-200 pb-3">
                  <h5 className="text-sm font-medium text-gray-500">{LABELS.leaseManagement.leaseTerm}</h5>
                  <p className="text-gray-800">{leaseSummary.startDate} - {leaseSummary.endDate}</p>
                </div>
                
                <div className="border-b border-gray-200 pb-3">
                  <h5 className="text-sm font-medium text-gray-500">{LABELS.leaseManagement.monthlyRent}</h5>
                  <p className="text-gray-800">{leaseSummary.monthlyRent}</p>
                </div>
                
                <div className="border-b border-gray-200 pb-3">
                  <h5 className="text-sm font-medium text-gray-500">{LABELS.leaseManagement.securityDeposit}</h5>
                  <p className="text-gray-800">{leaseSummary.securityDeposit}</p>
                </div>
                
                <div className="border-b border-gray-200 pb-3">
                  <h5 className="text-sm font-medium text-gray-500">{LABELS.leaseManagement.paymentDueDate}</h5>
                  <p className="text-gray-800">{leaseSummary.paymentDueDate}</p>
                </div>
                
                <div className="border-b border-gray-200 pb-3">
                  <h5 className="text-sm font-medium text-gray-500">{LABELS.leaseManagement.lateFee}</h5>
            ...
            <div className="md:w-1/3 relative">
              ...
              <div className="space-y-2 overflow-auto pb-20">
                {leases.map((lease) => (
                  <div
                    key={lease.id}
                    className={`p-3 rounded-lg cursor-pointer transition-colors ${selectedLease?.id === lease.id ? 'bg-blue-50 border border-blue-200' : 'bg-gray-50 hover:bg-gray-100 border border-gray-200'}`}
                    onClick={() => handleLeaseSelect(lease)}
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        <h4 className="font-medium text-gray-800">{lease.title}</h4>
                        <p className="text-xs text-gray-500 mt-1">{formatDate(lease.date)}</p>
                      </div>
                          ...
                          <div className="ml-2">
                            <label className="relative inline-flex items-center cursor-pointer">
                              <input
                                ...
                                onChange={() => {
                                  const newStatus = lease.leaseStatus === "ACTIVE" ? "PENDING" : "ACTIVE";
                              ...
                              <span className="ml-2 text-xs font-medium text-gray-700">
                                {lease.leaseStatus === "ACTIVE" ? "Active" : "Pending"}
                              </span>
                            </label>
                          </div>
                        )}
              ...
              {selectedLease ? (
                <div>
                  <div className="mb-4">
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                      {LABELS.leaseManagement.issuedOn} {formatDate(selectedLease.date)}
                    </p>
                  </div>
