...
export default function ParkingPassCard({
  ...
  return (
    <div
      className={`flex flex-col items-center bg-white shadow-2xl rounded-lg border-l-4 ${
        isExpired ? 'border-red-500' : 'border-primary-green'
      } w-full max-w-xl m-auto lg:m-4 my-4`}
    >
      <div
      ...
      >
        <Car
          className={`${
            isExpired ? 'bg-red-500' : 'bg-primary-green'
          } text-white rounded-full p-2`}
          size={50}
        />
        <h3 className='font-bold text-xl'>{LABELS.parkingPassCard.title}</h3>
        <span
          className={`${
            isExpired ? 'bg-red-500' : 'bg-primary-green'
      ...
      <div className='flex flex-col text-black items-center'>
        <div className='flex flex-row'>
          <div className='p-2 flex items-center'>
            <Calendar
              className={`${
                isExpired ? 'text-red-500' : 'text-primary-green'
              } p-2`}
              size={40}
            />
      ...
      <div className='p-4 flex gap-4'>
        {!isExpired ? (
          <>
            ...
            <button
              ...
              className={`flex flex-row justify-between ${
                isExpiring ? 'bg-gray-400' : 'bg-red-400'
              } py-1 px-4 rounded-md items-center text-white`}
            >
          ...
          <button
            ...
            disabled={isDeleting}
            className={`flex flex-row justify-between ${
              isDeleting ? 'bg-gray-400' : 'bg-red-400'
            } py-1 px-4 rounded-md items-center text-white`}
          >
            <X className='p-2' size={40} />
            {isDeleting ? 'Deleting...' : LABELS.parkingPassCard.deleteBtn}{' '}
            {parkingPassNumber && `#${parkingPassNumber}`}
          </button>
        )}
      </div>
